...
def _not_found(recency_log, sources, budget, today_iso, question):
    ...
    Records the search terms attempted so consumers can retry rather than
    treat the absence as a verified negative."""
    search_terms = []
    for s in sources or []:
...
def run(qas, today_iso):
    ...
    for q in qas:
        ...
        citations = [
            format_citation(s["url"], s["retrieval_iso"], s["source_title"], s["tier"])
